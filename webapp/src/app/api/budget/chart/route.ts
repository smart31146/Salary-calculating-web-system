...
import BudgetChart from "../schemas/chart";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const {chartName, pieChartName, userId} = json;
    // Create a new document using the Mongoose model
    const newChart = new BudgetChart({
      chartName,
      pieChartName,
      userId,
    });
   ...
   return new NextResponse(JSON.stringify({
    status: 'success',
    message: 'Budget Chart added successfully',
...
export async function GET(request: NextRequest) {
  await dbConnect();
    try {
      ...
      const allCharts = await BudgetChart.find();
