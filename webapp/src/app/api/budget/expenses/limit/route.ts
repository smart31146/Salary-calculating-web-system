...
import ExpenseLimit from "../../schemas/expenselimit";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const selectedChartExpense = await ExpenseLimit.find({ chartId: selectedChartId });
...
export async function POST(request: NextRequest) {
    try {
      ...
      const {upper, lower, categoryId, chartId} = json
      const newExpenseLimit = new ExpenseLimit({
        upper: Number(upper),
        lower: Number(lower),
        categoryId,
        chartId,
      ...
      const savedExpenseLimit = await newExpenseLimit.save();
     
     return new NextResponse(JSON.stringify({message: 'Expense Data Created', data: savedExpenseLimit}), {
          status: 201,
          headers: { "Content-Type": "application/json" },
        });
      }
  ...
  export async function DELETE(request: NextRequest) {
    ...
    if(type === 'soft'){
        const existingIncome = await ExpenseLimit.findById(expenseId);
        if (!existingIncome) {
        ...
        const result = await ExpenseLimit.deleteOne({ _id: expenseId });
...
export async function PATCH(request: NextRequest) {
    try {
      const json = await request.json();
      const {upper, lower, limitId} = json
      // Find the asset by ID
      const existingExpenseLimit = await ExpenseLimit.findById(limitId);
      // If the asset is not found, return an error response
      if (!existingExpenseLimit) {
        return new NextResponse(
          JSON.stringify({ message: 'ExpenseLimit not found' }),
          {
            status: 404,
            headers: { 'Content-Type': 'application/json' },
          }
      ...
      existingExpenseLimit.upper = Number(upper);
      existingExpenseLimit.lower = Number(lower);
      await existingExpenseLimit.save()
     return new NextResponse(JSON.stringify({message: 'Expense Limit Data Updated'}), {
          status: 201,
          headers: { "Content-Type": "application/json" },
        });
      }
